...
@section('place')
  ...
  <div id="frame" class="row wrapper border-bottom white-bg page-heading" style="padding:60px; display:none">
    ...
    <h1>Reporte General de Proyectos</h1>
        ...
        <tbody>
              ...
              <td class="color-red">{{ $total_conv }} ({{number_format($total_conv*100/$total_pjs)}}%)</td>
              <td class="color-orange">{{ $total_mono }} ({{number_format($total_mono*100/$total_pjs)}}%)</td>
          </tr>
        </tbody>
      </table>
		</div>
  ...
  <div>
    <table class="table " style="width:100%">
      ...
      <tbody>
        ...
        <tr>
          <td style="max-width:300px;">{{ substr($top->name,0,75) }}... <!--<a href="{{url('gotoproject/'.$top->id.'/'.$top->team.'/'.$top->cohort.'/'.$top->type)}}">Ir al proyecto</a>--></td>
          <td>{{ $top->type }}</td>
  ...
  <div style="margin-top:25px;">
    <a onclick="loadingReportPDF()" href="{{url('reprojects/')}}" class="btn btn-primary" style="margin:auto"><i class="fa fa-download"></i> Generar reporte general y catálogo de proyectos (PDF) </a>
  </div>
