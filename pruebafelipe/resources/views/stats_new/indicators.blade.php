    ...
    {!! Form::open(['class'=>'form-horizontal','method'=>'POST']) !!}
      ...
      <div id="subfilter">
        @if(Auth::user()!=null)
          ...
          @endif
        @endif  
        <div class="form-group"><label class="col-sm-2 control-label">Demarcación Administrativa</label>
          <div class="col-sm-10">
            <select class="js-example-basic-single form-control m-b" id="demAdm" style='width:100%'>
              <option value="0">Seleccione ...</option>
              <option value="1">Departamentos</option>
              <option value="2">Municipalidades</option>
              <option value="3">Nivel Nacional</option>
            </select>
          </div>
        </div> 

        <div id="depDiv" class="form-group"><label class="col-sm-2 control-label">Departamento</label>
        ...
        <div id="munDiv" class="form-group"><label class="col-sm-2 control-label">Municipio</label>

          <div class="col-sm-10">
              {!!Form::select('municipality',$municipality,null,['id'=>'municipality','class'=>'js-example-basic-single form-control m-b', 'placeholder' => 'Seleccione un municipio', 'required'=>'', 'style'=>'width:100%'])!!} 
          </div>

        </div>

        <div id="nNacional" class="form-group"> <h4> Bienes y servicios a nivel nacional </h4> </div>
     </div> 
      <div class="form-group"><label class="col-sm-2 control-label">Lista de bienes y servicios</label>

          <div class="col-sm-10">
            {!!Form::select('sel1',$result,null,['id'=>'sel1','class'=>'js-example-basic-single form-control m-b', 'placeholder' => 'Seleccione un bien o servicio', 'required'=>'', 'style'=>'width:100%'])!!} 
          </div>
...
<script type="text/javascript">
  ...
  function getReport(e) {
    ...
    nameReport = jQuery('select[name="sel1"] option:selected').text();
    if (filterRadio == 1) {
      //get report general
      if (nameReport == 'Seleccione un bien o servicio') {
        alert("Debe de seleccionar tipo de bien y servicio que desea visualizar");
        hideReport();
      } else {
        jQuery.ajax({
          url: '{{asset("stats/goodsservices/getStatsBS/typeReport")}}'.replace('typeReport',typeReport),
          type:"GET",
          dataType:"json",
      ...
      demAdm = jQuery('#demAdm').val();
      municipality = jQuery('#municipality').val();
      console.log("cohorte:-"+current_cohort+"-\n Superv:-"+supervisor+"-\n depar:"+departament); 
      if (demAdm == '0' || supervisor == '' || current_cohort == "Seleccione una cohorte" || nameReport == 'Seleccione un bien o servicio') {
        ...
        if(dataResult == 1){
          jQuery.ajax({
            url: '{{asset("stats/googsservices/getStatsBSFilters/typefilter/typeReport/gcohorts/supervisors/departament/yearsData/typeYears/dataYear/demadm")}}'.replace('typefilter/typeReport/gcohorts/supervisors/departament/yearsData/typeYears/dataYear/demadm', filterRadio+'/'+typeReport+'/'+current_cohort+'/'+supervisor+'/'+dataDem+'/createby/fsdb/34234/'+demAdm),
            type:"GET",
            dataType:"json",
            success:function(data){
      ...
      if (demAdm == '0' || supervisor == '' || year_data == "Seleccione una año" || datayear=='Seleccione ...' || filterRadioY=='' || nameReport == 'Seleccione un bien o servicio') {
        alert("Debe de seleccionar alguna opción en los filtros visualizados8888");
        hideReport();
      } else {
        if (demAdm == '1') {
          if (departament == '') {
        ...
        if(dataResult == 1){
          jQuery.ajax({
            url: '{{asset("stats/googsservices/getStatsBSFilters/typefilter/typeReport/gcohorts/supervisors/departament/yearsData/typeYears/dataYear/demadm")}}'.replace('typefilter/typeReport/gcohorts/supervisors/departament/yearsData/typeYears/dataYear/demadm', filterRadio+'/'+typeReport+'/cohort/'+supervisor+'/'+dataDem+'/'+year_data+'/'+filterRadioY+'/'+datayear+'/'+demAdm),
            type:"GET",
            dataType:"json",
            success:function(data){
              //console.log('console:'+data['result']);
      ...
      demAdm = jQuery('#demAdm').val();
      municipality = jQuery('#municipality').val();
      if (demAdm == '0' || supervisor == '' ||  nameReport == 'Seleccione un bien o servicio') {
        alert("Debe de seleccionar alguna opción en los filtros visualizados");
        hideReport();
      } else {
        if (demAdm == '1') {
        ...
        if(dataResult == 1){
          jQuery.ajax({
            url: '{{asset("stats/googsservices/getStatsBSFilters/typefilter/typeReport/gcohorts/supervisors/departament/yearsData/typeYears/dataYear/demadm")}}'.replace('typefilter/typeReport/gcohorts/supervisors/departament/yearsData/typeYears/dataYear/demadm', filterRadio+'/'+typeReport+'/cohort/'+supervisor+'/'+dataDem+'/Filters/Radio83$2/Dumss00/'+demAdm),
            type:"GET",
            dataType:"json",
            success:function(data){
              //console.log('console:'+data['result']);
...
//Funciones de filtros de año
  ...
  function hideDemAdm() {
    jQuery("#depDiv").hide();
    jQuery("#munDiv").hide();
    jQuery("#nNacional").hide();
  }
  var dem;
  function showSelectDemAdm() {
    dem = jQuery("#demAdm").val();
    if (dem == 1) {
      hideDemAdm();
      jQuery("#depDiv").show();
    }else if(dem == 2){
  ...
  jQuery(document).ready(function() {
    ...
    jQuery("#rTime").hide();
    hidefilterY();
    jQuery("#demAdm").change(showSelectDemAdm);
